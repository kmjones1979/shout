  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (value && value.trim()) {
        ...
        cleanedData[key as keyof SocialLinks] = value.trim().replace(/^@/, "");
      }
    });
                ...
                <div key={platform.key}>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full py-2.5 pl-8 pr-4 bg-zinc-800 border border-zinc-700 rounded-xl text-white placeholder:text-zinc-600 focus:outline-none focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 transition-all text-sm"
                    />
                  </div>
                </div>
              ))}
