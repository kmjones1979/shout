...
const FRIENDS_STORAGE_KEY = "shout_friends";
...
export function useFriends(userAddress: Address | null) {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem(`${FRIENDS_STORAGE_KEY}_${userAddress}`);
    if (stored) {
      try {
  ...
  const addFriend = useCallback(
    async (input: string, nickname?: string): Promise<boolean> => {
      ...
      try {
        ...
        if (friends.some((f) => f.address.toLowerCase() === resolution.address!.toLowerCase())) {
          setError("This address is already in your friends list");
          return false;
        }

        // Check if trying to add self
        if (resolution.address.toLowerCase() === userAddress.toLowerCase()) {
