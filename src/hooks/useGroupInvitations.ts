...
export function useGroupInvitations(userAddress: string | null) {
  ...
  const sendInvitations = useCallback(
    async (groupId: string, groupName: string, inviteeAddresses: string[]): Promise<boolean> => {
      ...
      try {
        ...
        const { error } = await supabase
          .from("shout_group_invitations")
          .upsert(invitations, { onConflict: "group_id,invitee_address" });
  ...
  const acceptInvitation = useCallback(
    async (invitationId: string): Promise<{ success: boolean; groupId?: string; error?: string }> => {
